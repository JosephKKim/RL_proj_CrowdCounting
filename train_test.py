            ...
            if not replay.can_sample():
                ...
                while rest>0:  
                    ...
                    if rest < parameters['TRAIN_SKIP']: 
                        ...
                        # for Double DQN
                        argmax_Q = eval_Q.data.max(1)[1].unsqueeze(1)
                        double_newQ = newQ.data.gather(1, argmax_Q)
                        
                        # Double DQN
                        target_Q = double_newQ * parameters['GAMMA'] * (1 - done_mask) + rew_batch 
...
def train_model_sche(net, epoch, all_epoches, train_path, replay, optimizer, scheduler, minerror, parameters):
